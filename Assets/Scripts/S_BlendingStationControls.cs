...
//		}
	...
	public void ClearBlender()
	{
		...
		if (GameObject.Find ("ShakingStationCamera").GetComponent<S_ShakingStationControls> ().somethingInJug == false)
		{
			if (gameManager.GetComponent<S_InventoryManager> ().currentUnshakenLemonTwist == 1) // drink is lemon twist
			{
			...
			blendButton.SetActive (false);
			canMakeNothing = true;
			GameObject.Find ("ShakingStationCamera").GetComponent<S_ShakingStationControls> ().somethingInJug = true;
		}
	}
